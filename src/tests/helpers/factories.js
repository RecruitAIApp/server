...
export const createClosedJob = async (companyId, postedById) => {
  return await createTestJob(companyId, postedById, {
    status: "closed",
    applicationDeadline: new Date(Date.now() - 100000) // Past deadline
  });
};
...
export const createTestCandidate = async (withResume = true) => {
  ...
  const resumeData = withResume ? {
    resume: {
      url: "https://res.cloudinary.com/dcrjaawc6/raw/upload/v1780747493/recruiter_ai/resumes/cv_1780747492250_Hafsa_Ahmed.pdf",
      publicId: "resume_123",
      fileName: "resume.pdf",
      parsedData: {
